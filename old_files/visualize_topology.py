...
def draw_topology(G, pos):
    ...
    plt.title("SDN Topology", fontsize=16)
    plt.axis('off')
    plt.tight_layout()
    plt.pause(0.001)
...
def live_visualizer(csv_path, refresh_interval=1):
    plt.figure("Live Running Network")
    while True:
...
def run_live_visualization(csv_path):
    # Starts the visualization thread and blocks the main thread
...
if __name__ == "__main__":
    try:
        if os.path.exists(INITIAL_PATH):
            ...
            print(f"[WARN] Missing {INITIAL_PATH}, skipping initial visualization.")

        if os.path.exists(CSV_PATH):
            print("[INFO] Starting live visualization of running network in background...")
            start_visualization_thread(CSV_PATH)
